...
uh oh we need a tsibble ts-data frame
...
toolik_ts <- toolik_df %>% 
  ...
  as_tsibble(key = NULL, ### if we had multiple obs on same date from diff locations #key is county or whatever other grouping we need
...
Way more easyily to filter dates
...
```{r}
toolik_ts |> filter_index("2010-12" ~ "2011-01-15") 
...
```{r}
toolik_ts |> filter_index("2006-04-10" ~ "2006-05-15") 
```
...
```{r}
toolik_ts |> filter_index("2020-12-20" ~ "2022-12-31") 

#or solve this way

toolik_ts |> filter_index("2020-12-20" ~ .) 
```
...
```{r}
toolik_ts |> filter_index("2010-12" ~ "2011-01", "2013-09" ~ "2014-10-31" ) 
```


## Use `index_by()` to aggregate time series by increments

**index by replaces groupby() with index_by()**
...
toolik_month <- toolik_ts |>  
  index_by(yr_mo = ~yearmonth(.)) |> #at what measure do we want to look at our dates? 
...
**the tilda tries to pass in inforamtion, not clear when you want to.. tricky. Pass info in... (faucet wrap just uses,)  DOT means everything in the data frame... 
